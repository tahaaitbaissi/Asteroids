...
void DrawHealth(void)
{
    ...
    Rectangle r = (Rectangle){ 
        (SCREEN_WIDTH * 0.5f) - (barWidth * 0.5f),
        SCREEN_HEIGHT - cellHeight * 1.5f,
        barWidth,
        cellHeight
    };
    ...
    Color color = (health >= PLAYER_HEALTH_MAX * 0.6f)
                  ? healthHigh : (health >= PLAYER_HEALTH_MAX * 0.25f)  
                  ? healthMid
                  : healthLow;
